...
Краткое введение в платформу транспортного моделирования MATSim

---

## 1. Что такое MATSim

**MATSim** (Multi-Agent Transport Simulation) - это **open-source платформа** для имитации транспортного спроса и потоков. Её используют научные группы, консультанты и органы управления в разных странах для оценки сценариев развития транспорта и территорий. Примеры: Париж, Берлин, Вена *https://matsim.org/examples/*

**Берлин**

![Пример применения MATSim: транспортная модель Берлина](../examples/berlin_1.jpg)


- **Открытый исходный код** - можно адаптировать под задачу и верифицировать логику, документация и проекты других команд находятся в отркытом доступе
...
Каждый **агент** в модели - это упрощённое представление человека. У агента есть **план на день**: последовательность действий и поездок.
...
Маршруты агентов строятся по графу, поэтому качественная и детализированная сеть напрямую влияет на реалистичность результатов. Ниже - пример визуализации атрибутов сети.

![Обзор свободной скорости по линкам сети - Шамалган](../../Visualization/network-qc/network_speed_overview.png)

*Свободная скорость (free-speed) по линкам: тёмно-синий - низкая, красный - высокая.*

### 2.3. Общественный транспорт (ОТ)

**Схема общественного транспорта**

![Схематичная карта ОТ](../../Visualization/pt-data/pt_schematic.png)

Общественный транспорт в MATSim задаётся:

- **Остановками** - Пункты остановок, привязанные к траснпортным отрезкам.
...
**Как модель ищет устойчивое решение**

![Сходимость симуляции - рост средней полезности и стабилизация](../../Visualization/docs-progress/simulation_convergence.png)

*Пример (сценарий Шамалган, 100 итераций): доли режимов и средняя полезность по `modestats.csv` / `scorestats.csv`; при сходимости доли и полезность перестают систематически «дрейфовать» от итерации к итерации.*
...
Чем выше итоговая полезность, тем «лучше» для агента такой вариант дня.

### 2.5. Итерации и перепланирование

Один **шаг** модели - это расчёт всех агентов по их текущим планам в общей сети (один «виртуальный день»). После этого подсчитываются задержки, время в пути, полезность.

**Итерация** обычно включает:

1. **Выполнение** планов (симуляция дня).
2. **Оценку** результатов (scoring).
3. **Перепланирование** - часть агентов меняет планы (например, способ поездки или время выезда), чтобы попробовать «лучший» вариант в следующем расчёте.
...
| Сравнение сценариев до и после застройки | Сравнительный анализ интенивности, загрузки ОТ и прочих параметров. |
...
- **Население** задано по зонам и привязано к сети (дома и рабочие места на линках).
