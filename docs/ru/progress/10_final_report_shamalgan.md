...
Цель проекта — отработать полный цикл: от подготовки исходных данных до запуска агентной симуляции с общественным транспортом, выбором режимов и визуализацией результатов.
...
Исходные пространственные данные территории собраны в файл **`new_map.gpkg`** формата **GeoPackage** (OGC-стандарт на базе SQLite). Это **не** шейп-файл и не `.osm` — формат был выбран за компактность и поддержку нескольких слоёв в одном файле.

**Слои GeoPackage:**

| Слой | Тип геометрии | Содержание |
|------|---------------|------------|
| `map__multilinestrings` | Линии | Осевые линии улично-дорожной сети (в т.ч. двойные линии единственной магистрали в селе) |
| `New_Points` | Точки | Коммерческие объекты (магазины, заправки), а также остановки ОТ с атрибутами маршрутов |
| Полигоны | Полигоны | Границы застройки и территории (на данном этапе **не используются**) |
...
> **Примечание.** Разметка маршрутов через атрибуты точек — вынужденный приём, позволивший быстро описать маршрутную сеть без GTFS. В следующем проекте планируется использовать штатные инструменты (GTFS, pt2matsim).
...
**Итоговые параметры сети:** 1 128 узлов, 2 991 звено, одна связная компонента, ноль тупиков и изолированных узлов.

**Замечание о геометрии.** При визуальном сравнении геометрия звеньев из OSM-шейпа заметно упрощена по сравнению с реальной трассировкой улиц: дуги спрямлены, повороты сглажены. Для proof-of-concept это не критично, однако для повышения точности (например, при оценке длин перегонов ОТ) геометрию целесообразно уточнить.


### 2.3. Распределение населения по сети

**Задача:** разместить синтетических транспортных агентов (представляющих жителей) на звеньях дорожной сети.

**Исходные данные.** Использована карта плотности населения территории. На её основе скрипт `derive_shamalgan_zones.py` делит территорию на **прямоугольную сетку 10×8** (80 ячеек). Каждая ячейка — транспортный район (зона) размером порядка **300 м по каждому измерению** (зависит от протяжённости территории). Из 80 ячеек активны только те, где плотность населения > 0 (пустые отбрасываются).
...
2. Внутри зоны агент размещается **с вероятностью, пропорциональной длине звена**: среди всех звеньев, попадающих в круг радиуса `sigma_m` вокруг центра зоны, выбирается звено с вероятностью ~длина/сумма длин. Затем координата агента — случайная точка на этом звене. Это предотвращает сгущение агентов на коротких тупиковых звеньях.
3. **Запасной механизм:** если в радиусе `sigma_m` нет ни одного звена, координата определяется гауссовым разбросом (`sigma_m` — стандартное отклонение) вокруг центра зоны, после чего агент привязывается к ближайшему звену сети (`NetworkUtils.getNearestLinkExactly`).
...
- **Маршрут 213:** задан обязательный коридор (ключевое звено 2011) — маршрут должен пройти через это звено. Одновременно заблокированы звенья у железнодорожного переезда, через которые реальный автобус не проходит.
- **Маршрут 256:** допустимы только звенья с определёнными `origid` — маршрутка ходит по узкому коридору, а не по магистрали.

**Привязка остановок.** Каждая остановка «прикреплена» к ближайшему звену сети (snap). Максимальное расстояние snap — 20,6 м; остановок с расстоянием > 80 м — ноль. Недостижимых сегментов — ноль.
...
*Карта маршрутов ОТ на дорожной сети (трассы и остановки):*

![Карта маршрутов ОТ на дорожной сети — Шамалган](img/pt_routes_map.png)
...
├── network-with-pt.xml      ← Сеть + псевдозвенья ОТ
...
|-------------|---------|-----------|
| Упрощённая геометрия звеньев (спрямление дуг) | Неточные длины перегонов | Средний |
...
| `Visualization/population_on_network_map.html` | Карта распределения населения по звеньям |

### Необходимые визуализации (для презентации и Notion)

| № | Визуализация | Статус | Промпт для визуализатора |
|---|-------------|--------|--------------------------|
| 1 | Карта УДС с цветовой кодировкой по скорости/полосности | Нужна | Построить карту network.xml: звенья окрашены по freespeed (шкала), толщина — по числу полос |
...
| 3 | Карта распределения агентов по звеньям (обновлённая PNG) | Нужна | Перестроить infographic из population_by_link.csv на фоне network.xml |
