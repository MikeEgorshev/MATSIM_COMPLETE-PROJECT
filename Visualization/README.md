...
- **`population_on_network_map.html`** — распределение активностей (дом/работа/другое) по звеньям сети (подложка OSM). Генерация: `python tools/build_population_on_network_infographic.py --html`.
...
| **`population_on_network_map.html`** | **`python tools/build_population_on_network_infographic.py --html`** |
